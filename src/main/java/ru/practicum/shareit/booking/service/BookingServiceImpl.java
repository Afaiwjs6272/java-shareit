...
public class BookingServiceImpl implements BookingService {
    ...
    public List<BookingDto> getUserBookings(Long userId, State state) {
        User user = userExistCheckAndLoad(userId);
        Iterable<Booking> result = bookingRepository.findAll(BookingPredicate.bookerAndState(user.getId(), state),
    ...
    @Override
    public List<BookingDto> getUserItemBookings(Long userId, State state) {
        User user = userExistCheckAndLoad(userId);
        Iterable<Booking> result = bookingRepository.findAll(BookingPredicate.itemOwnerAndState(user.getId(), state),
                BookingPredicate.orderByStart());
        return StreamSupport.stream(result.spliterator(), false).map(BookingMapper::toDto).toList();
    }
