...
public class ItemServiceImpl implements ItemService {
    ...
    public List<ItemOwnerDto> getItemsByOwnerId(Long userId) {
        ...
        Iterable<Booking> bookings = bookingRepository.findAll(BookingPredicate.itemOwnerAndState(userId, State.ALL),
                BookingPredicate.orderByStart());
    ...
    public List<ItemDto> search(Long userId, String query) {
        ...
        return itemRepository.findByText(query).stream().map(ItemMapper::toItemDto).toList();
    }

    @Override
    public ItemDto getItem(Long userId, Long id) {
        ItemDto item = ItemMapper.toItemDto(itemRepository.findById(id).orElseThrow(() -> new ItemNotFoundException("This item doesn't exist")));
